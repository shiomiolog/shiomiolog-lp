<template>
  <main
    ...
    :class="isMeowMode ? 'sepia-[0.2] hue-rotate-[340deg] cursor-none' : 'bg-slate-100'" @mousemove="handleMouseMove"
    @click="handleGlobalClick">
    <div v-if="isMeowMode" class="fixed pointer-events-none z-[999] text-3xl transition-all duration-500 ease-out"
      :style="{ left: mouseX + 'px', top: mouseY + 'px', transform: 'translate(-50%, -50%)' }">🐾</div>

    <div class="absolute inset-0 flex items-center justify-center z-50 pointer-events-none">
      <div class="flex flex-col items-center">
        <button @click.stop="handleAvatarClick"
    ...
    <NuxtLink :to="links.logical" target="_blank"
      class="relative flex-1 bg-white group border-r border-slate-100 flex flex-col items-center justify-center p-8 transition-all duration-700 hover:flex-[1.2]">
      <div
        class="absolute inset-0 flex items-center justify-center opacity-[0.03] group-hover:opacity-[0.07] transition-opacity pointer-events-none">
      ...
      <div class="z-10 text-center max-w-xs transition-transform duration-500 group-hover:-translate-y-4">
        ...
        <h2
          class="text-4xl md:text-6xl font-black text-slate-200 group-hover:text-blue-600 transition-all uppercase tracking-tighter mb-4 italic">
          {{ isMeowMode ? 'Meowgical' : 'Logical' }}
        </h2>

        <div
          class="space-y-2 opacity-0 group-hover:opacity-100 transition-all duration-700 delay-100 transform translate-y-4 group-hover:translate-y-0">
          ...
          <div class="flex flex-wrap justify-center gap-2 mt-4">
            <span
              class="text-[10px] px-2 py-0.5 border border-blue-100 text-blue-400 rounded uppercase">Entertainment</span>
            <span class="text-[10px] px-2 py-0.5 border border-blue-100 text-blue-400 rounded uppercase">Creative</span>
            <span
              class="text-[10px] px-2 py-0.5 border border-blue-100 text-blue-400 rounded uppercase">Broadcasting</span>
          </div>
        </div>
      </div>
    </NuxtLink>

    <NuxtLink :to="links.physical" target="_blank"
      class="relative flex-1 bg-slate-900 group flex flex-col items-center justify-center p-8 transition-all duration-700 hover:flex-[1.2]">
      <div
        class="absolute inset-0 flex items-center justify-center opacity-[0.05] group-hover:opacity-[0.1] transition-opacity pointer-events-none">
      ...
      <div class="z-10 text-center max-w-xs transition-transform duration-500 group-hover:-translate-y-4">
        ...
        <h2
          class="text-4xl md:text-6xl font-mono font-bold text-slate-800 group-hover:text-emerald-400 transition-all uppercase tracking-tighter mb-4">
          {{ isMeowMode ? 'Pawsical' : 'Physical' }}
        </h2>

        <div
          class="space-y-2 opacity-0 group-hover:opacity-100 transition-all duration-700 delay-100 transform translate-y-4 group-hover:translate-y-0">
          <p class="text-emerald-400 font-mono text-lg">{{ isMeowMode ? 'システムの守護猫' : '物理アバター 自己紹介' }}</p>
          ...
          <div class="flex flex-wrap justify-center gap-2 mt-4 font-mono">
            <span
              class="text-[10px] px-2 py-0.5 border border-emerald-900 text-emerald-700 rounded uppercase">Low-Layer</span>
            <span
              class="text-[10px] px-2 py-0.5 border border-emerald-900 text-emerald-700 rounded uppercase">Security</span>
            <span
              class="text-[10px] px-2 py-0.5 border border-emerald-900 text-emerald-700 rounded uppercase">Kernel</span>
          </div>
        </div>
      </div>
    </NuxtLink>
...
const profileSpecs = {
  "name (legal)": "See academic portal (https://mayu2664.jp/)"
};
